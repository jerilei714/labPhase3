...
const {
  ...
  updateReservation,
  deleteReservation,
  getReservedSeatsByUsername,
  getReservationsByUsername
} = require('../Model/labReserves');
const {updateSeatStatus} = require('../Model/labSeats');
const {createReservedSeat, checkSeatAvailability} =
    require('../Model/labReservedSeats');
...
router.put('/update/:reservationId', async (req, res) => {
  try {
    ...
    const success =
        await updateReservation(new ObjectId(reservationId), updatedDetails);
    if (success) {
...
router.get('/byUsername/:username', async (req, res) => {
  try {
    ...
    const userReservations =
        await getReservationsByUsername(username, labId, reserveDate);
    res.json({userReservations});
  } catch (error) {
    console.error('Error fetching user reservations:', error);
    res.status(500).json({error : 'Internal server error'});
...
/* router.delete('/:reservationId', async (req, res) => {
    try {
        ...
        const reservationTime = new Date(reservation.reserve_date + 'T' +
   reservation.reserve_time); const timeDifference = (reservationTime -
   currentTime) / (1000 * 60); */ // diff in minutes
...
/*             res.status(400).json({ error: 'Cannot remove reservation outside
