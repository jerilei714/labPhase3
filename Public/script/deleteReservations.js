...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function generateSeats(seatContainer, seatCount) {
    ...
    for (let i = 1; i <= seatCount; i++) {
      ...
      seat.classList.add('seat');
      seat.innerText = i;
      seat.addEventListener('click', function() { showPopup(seat); });
      seatContainer.appendChild(seat);
    }
  }
  ...
  async function viewAvailability() {
    try {
      ...
      if (currentTime >= reservationStartDateTime) {
        ...
        // is past the buffer
        const response = await fetch(`/seats/available/${currentLab}?date=${
            encodeURIComponent(selectedDate)}&startTime=${
            encodeURIComponent(
                startTime)}&endTime=${encodeURIComponent(endTime)}`);
        if (!response.ok) {
          throw new Error('Failed to fetch available seats');
        }
        const availableSeatCount = await response.json();
        ...
        availabilityResults.innerHTML = `<h3>${
            currentLab} Availability</h3><p class="Available">Available Seats: ${
            availableSeatCount}</p>`;
        availabilityResults.style.display = 'block';
        const seatContainer = document.createElement('div');
        seatContainer.classList.add('seat-container');
        const labInfoResponse =
            await fetch(`/labs/name/${encodeURIComponent(currentLab)}`);
        ...
        const reservedSeatsResponse = await fetch(
            `/reservedseats/lab/${currentLab}?date=${selectedDate}&startTime=${
                encodeURIComponent(
                    startTime)}&endTime=${encodeURIComponent(endTime)}`);
        ...
        availabilityResults.innerHTML =
            `<p>Reservations will be displayed 10 minutes past their start time.</p>`;
        availabilityResults.innerHTML =
            `<p>You can only delete reservations 10 minutes past their start time.</p>`;
        availabilityResults.style.display = 'block';
      }
    } catch (error) {
      console.error('Error fetching available seats:', error);
  ...
  function showPopup(seat) {
    ...
    fetch(`/reservations/${decrementedId}`)
        ...
        .then(reservation => {
          ...
          document.querySelector('#date-reserved').innerHTML =
              new Date(reservation.tnd_requested)
          ...
          if (reservation.username != "Anonymous") {
            anchorElement.classList.add("userName")
            anchorElement.href = `viewProfile?username=${
                encodeURIComponent(reservation.username)}`
          } else {
            userNameElement.classList.remove("userName")
          }
          popup.style.display = 'flex';
  ...
  document.getElementById('buttonForAvailibility')
      .addEventListener('submit', function(event) {
        event.preventDefault();
        viewAvailability();
      });

  window.hideIt = function() { hidePopup(); };
});
